...
describe('sonar script', function () {
  ...
  it(` should fail when sonar-project.properties is missing`, async () => {
    ...
    await shouldFail(() => sonarScript.run(), err => err.message === "ENOENT: no such file or directory, open 'sonar-project.properties'");
...
`);
  ...
  validPropertyFiles.forEach((propertyFile) => {
    describe(` when using "${propertyFile}" valid file`, () => {
      ...
      it(` should fail when there is no local git repository`, async () => {
        ...
        await shouldFail(() => sonarScript.run(), err => err.message === 'Expected success codes were "0", but the process exited with "128".');
        ...
        assertText(loggerRecorder.recordedLogs, [
          ...
          {
            kind: 'endsWith',
            value: 'error: Script "sonar" failed after 0 s with: Expected success codes were "0", but the process exited with "128".\n',
          },
        ]);
      ...
      describe(' when project already exists in Sonar', () => {
        ...
        it(` should succeed when simple code analysis succeeds.`, async () => {
          ...
          assert.isTrue(shellCommand.calledWithExactly(SONAR_SCANNER, [
        ...
        it(` should fail when simple code analysis fails.`, async () => {
          ...
          await shouldFail(() => sonarScript.run(), err => err.message === 'An error occurred while analyzing source code.');
          ...
          assertText(loggerRecorder.recordedLogs, [
            ...
            {
              kind: 'endsWith',
              value: 'error: Script "sonar" failed after 0 s with: An error occurred while analyzing source code.\n',
            },
          ]);
      ...
      describe(' when project does not yet exist in Sonar', () => {
        ...
        it(` should fail when Sonar project initialization fails.`, async () => {
          ...
          await shouldFail(() => sonarScript.run(), err => err.message === 'An error occurred while calling sonar-init sub-script.');
          ...
          assertText(loggerRecorder.recordedLogs, [
            ...
            {
              kind: 'endsWith',
              value: 'error: Script "sonar" failed after 0 s with: An error occurred while calling sonar-init sub-script.\n',
            },
            {
              kind: 'contains',
              invert: true,
  ...
  describe(' when using a sonar-project.properties file where Sonar host is missing', () => {
    ...
    it(` should fail with a message about missing host url.`, async () => {
      ...
      await shouldFail(() => sonarScript.run(), err => err.message === '"sonar.host.url" property must be defined in "sonar-project.properties" file!');
  ...
  describe(' when using a sonar-project.properties file where Sonar project key is missing', () => {
    ...
    it(` should fail with a message about missing project key.`, async () => {
      ...
      await shouldFail(() => sonarScript.run(), err => err.message === '"sonar.projectKey" property must be defined in "sonar-project.properties" file!');
